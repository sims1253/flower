...
from flower.models.memory import causal_running_mean
...
class PartitionedMemoryRead(nn.Module):
    ...
    def forward(self, x: torch.Tensor, memory: torch.Tensor, bank_log_weights: torch.Tensor) -> torch.Tensor:
        if memory.dim() == 5:
            # causal_memory=True: per-position memory (B, T, N, S, D);
            # bank_log_weights: (B, T, N). The token at t reads only the
            # bank state at t.
            return self._forward_causal(x, memory, bank_log_weights)
    ...
    def _forward_causal(
        self, x: torch.Tensor, memory: torch.Tensor, bank_log_weights: torch.Tensor
    ) -> torch.Tensor:
        """Per-position read for causal_memory=True. Same parameters as the
        legacy read; the (T, 1, N*S) attention at each position is restricted
        to that position's bank state."""
        bsz, seq_len, num_banks, slots_per_bank, dim = memory.shape
        mem_flat = memory.reshape(bsz, seq_len, num_banks * slots_per_bank, dim)
        q = self._split(self.q(x))  # (B, H, T, hd)
        k, v = self.kv(mem_flat).chunk(2, dim=-1)  # (B, T, N*S, D) each
        k = k.reshape(bsz, seq_len, -1, self.num_heads, self.head_dim).permute(0, 3, 1, 2, 4)  # (B,H,T,J,hd)
        v = v.reshape(bsz, seq_len, -1, self.num_heads, self.head_dim).permute(0, 3, 1, 2, 4)
        scores = q.unsqueeze(-2) @ k.transpose(-2, -1) / math.sqrt(self.head_dim)  # (B, H, T, 1, J)
        # Additive router bias, per position: (B, T, N) → (B, 1, T, 1, N*S).
        bias = bank_log_weights.repeat_interleave(slots_per_bank, dim=-1)  # (B, T, N*S)
        scores = scores + bias.unsqueeze(1).unsqueeze(-2)
        attn = torch.softmax(scores, dim=-1)
        out = attn @ v  # (B, H, T, 1, hd)
        out = out.squeeze(-2).transpose(1, 2).contiguous().view(x.shape)
        return self.out(out)
...
class PartitionedMemoryBlock(nn.Module):
    ...
    def _initial_memory_causal(self, x: torch.Tensor) -> torch.Tensor:
        """(B, T, N, S, D) per-position bank state for causal_memory=True."""
        return x.new_zeros(x.shape[0], x.shape[1], self.num_banks, self.slots_per_bank, self.config.d_model)

    def _bank_weights(self, x: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        """Return (soft_weights, log_weights) of shape (B, N) — or (B, T, N)
        when causal_memory=True (per-position routing from the prefix-mean).
        ...
        """
        pooled = causal_running_mean(x) if self.config.causal_memory else x.mean(dim=1)  # (B, D) / (B, T, D)
    ...
    def _update_memory(self, memory: torch.Tensor, x: torch.Tensor, bank_weights: torch.Tensor) -> torch.Tensor:
        ...
        # token_summary: per-bank max-pool aggregation (prefix max when causal),
        # broadcast to slots_per_bank.
        token_summary = torch.cummax(x, dim=1).values if causal else x.max(dim=1, keepdim=True).values
        if causal:
            token_summary = token_summary.unsqueeze(2)  # (B, T, 1, D)
        ...
        # Build (B, N, S, D) / (B, T, N, S, D) candidate update.
        summary = self.token_mlp(token_summary)
        summary = summary.unsqueeze(3) if causal else summary.unsqueeze(1)  # (B, T, 1, 1, D) / (B, 1, 1, D)
        candidate = self.update(
            summary
            + self.mem_mlp(memory)  # (B, N, S, D) / (B, T, N, S, D)
        ...
        # (B, N) → (B, N, 1, 1) legacy; (B, T, N) → (B, T, N, 1, 1) causal.
        scale = bank_weights.unsqueeze(-1).unsqueeze(-1)
    ...
    def forward(self, x: torch.Tensor, memory: torch.Tensor | None = None) -> tuple[torch.Tensor, torch.Tensor]:
        # Legacy memory is (B, N, S, D); causal memory is (B, T, N, S, D). Both
        # carry the bank count on dim -3.
