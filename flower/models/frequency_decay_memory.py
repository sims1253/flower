...
class FrequencyDecayLM(CausalLM):
    ...
    Everything the base flags own is routed through the parent helpers so the
    flags are live here too (pullfrog review of PR #11: the attributes existed
    but the forward bypassed the helpers, silently no-opping
    ``fp8_lm_head``/``bf16_cross_entropy``/``fused_linear_ce``/
    ...
    * blocks are wrapped in ``torch.utils.checkpoint`` mirroring base.py's
    ...
    def forward(self, input_ids: torch.Tensor, labels: torch.Tensor | None = None) -> dict[str, Any]:
        ...
        # handles the (x, state-tuple) signature: args are passed through
        # untouched and RNG state is saved/restored, so dropout and the
        # deterministic write_mag accumulator recompute identically in
        # backward. Skipped in eval, when the flag is off, and for
        # loop_count > 1 (same documented limitation as base.py). "ffn" has no
        # clean FFN-only boundary here — the memory update interleaves with
        # attention/FFN — so it falls back to FULL checkpointing with a
        # one-time notice, exactly the contract base.py applies to
        # non-TransformerBlock blocks.
        do_ckpt = (
            self.training
        ...
        if do_ckpt:
            ...
            if self.activation_checkpoint == "selective":
                ...
                for block in self.blocks:
                    ...
                    x, state = checkpoint(block, x, state, use_reentrant=False)
        else:
